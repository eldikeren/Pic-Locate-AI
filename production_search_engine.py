"""
Production Search Engine: Retrieval + Verification
Stage A: Fast candidate retrieval (SQL + pgvector)
Stage B: VLM verification with final AI verdict
Stage C: Re-rank and filter by AI confidence
"""
...
# import aiohttp  # Optional for async HTTP requests
...
@dataclass
class VLMVerdict:
    """VLM verification result with strict JSON schema"""
    image_id: str
    matches: bool
    confidence: float
    room: str
...
class ProductionSearchEngine:
    """Production-ready search engine with AI final verdict"""
    ...
    def __init__(self, openai_api_key: str, supabase_client):
        ...
        self.CUTOFF = 0.7  # VLM confidence threshold
        ...
        # VLM System Prompt with strict JSON schema
  ...
  "verdicts": [
    {
      ...
      "matches": true/false,
      "confidence": 0.0-1.0,
      "room": "kitchen|dining_room|living_room|bedroom|bathroom|office|hallway|balcony|garden|unknown",
      "evidence": {
        "objects": ["string"],
        "colors": {"object": "color"},
    ...
    async def search(self, query: str, lang: str = "en", limit: int = 24) -> List[FinalResult]:
        """
        Main search function: Retrieval + Verification
        ...
        """
        print(f"🔍 Production Search: '{query}' (lang: {lang})")
        ...
        print("📊 Stage C: Re-rank and filter...")
        final_results = self._stage_c_ranking(verdicts, candidates, limit)
    ...
    async def _normalize_query(self, query: str, lang: str) -> str:
        ...
        if lang == "he":
            query_en = query
            for he, en in self.he_to_en.items():
    ...
    async def _stage_a_retrieval(self, query_en: str, top_k: int) -> List[CandidateImage]:
        """Stage A: Fast candidate retrieval using SQL + pgvector"""
        try:
            # Parse query for exact filters
            room_filter = self._extract_room_filter(query_en)
            object_filters = self._extract_object_filters(query_en)
            ...
            if room_filter:
                where_conditions.append("i.room_type = $1")
            else:
                where_conditions.append("$1::text IS NULL")
            
            # Add object filters
            if object_filters:
                for obj in object_filters:
                    param_count += 1
                    where_conditions.append(f"EXISTS (SELECT 1 FROM image_objects o WHERE o.image_id = i.id AND o.label ILIKE ${param_count})")
                    params.append(f'%{obj}%')
    ...
    def _stage_c_ranking(self, verdicts: List[VLMVerdict], candidates: List[CandidateImage], limit: int) -> List[FinalResult]:
        """Stage C: Re-rank and filter results by AI confidence"""
    ...
    def _extract_match_reasons(self, evidence: Dict[str, Any]) -> List[str]:
        ...
        if objects:
            reasons.append(f"Objects: {', '.join(objects)}")
        
        # Colors
        colors = evidence.get('colors', {})
        if colors:
            color_desc = [f"{obj}={color}" for obj, color in colors.items()]
            reasons.append(f"Colors: {', '.join(color_desc)}")
        
        # Materials
        materials = evidence.get('materials', {})
        if materials:
            material_desc = [f"{obj}={material}" for obj, material in materials.items()]
    ...
    def _extract_object_filters(self, query: str) -> List[str]:
        """Extract object filters from query"""
        object_keywords = [
            'table', 'chair', 'sofa', 'bed', 'sink', 'toilet', 'tv', 'television',
            'refrigerator', 'fridge', 'oven', 'stove', 'microwave', 'dishwasher',
            'cabinet', 'wardrobe', 'dresser', 'nightstand', 'coffee table',
            'dining table', 'armchair', 'bookshelf', 'mirror', 'lamp'
        ]
...
async def test_production_search():
    """Test the production search engine"""
    print("Testing Production Search Engine...")
    
    # This would be initialized with actual API keys and Supabase client
    # search_engine = ProductionSearchEngine(openai_api_key="your-key", supabase_client=supabase)
    
    test_queries = [
        ("kitchen with black table", "en"),
        ("bathroom with marble countertop", "en"), 
        ("living room with large sofa", "en"),
        ("מטבח עם שולחן שחור", "he"),  # Hebrew
        ("שירותים עם שיש", "he")  # Hebrew
    ]
    
    for query, lang in test_queries:
        print(f"\nQuery: '{query}' (lang: {lang})")
        # results = await search_engine.search(query, lang)
        ...
        #     print(f"AI notes: {top_result.ai_notes}")
